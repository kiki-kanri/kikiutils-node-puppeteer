...
export const getSelectorAttribute = async (el: ElementHandle | Page, selector: string, attributeName: string) => {
	const element = await el.$(selector);
	return element && getElementAttribute(element, attributeName);
}
